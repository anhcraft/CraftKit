...
public class PlayerBackend extends CBModule implements CBPlayerBackend {
    ...
    public void setProfile(Player player, GameProfile profile) {
        ReflectionUtil.setDeclaredField(EntityHuman.class, ((CraftPlayer) player).getHandle(), "bT", profile);
    }
    ...
    public void changeSkin(Player player, Skin skin, List<Player> viewers) {
        ...
        craftServer.getHandle().moveToWorld(ent, cw.getHandle().dimension, true, player.getLocation(), true);
    }
    ...
    public void openBook(Player player, int slot) {
        ...
        buf.setByte(0, slot);
        buf.writerIndex(1);
        sendPacket(new PacketPlayOutCustomPayload("MC|BOpen", new PacketDataSerializer(buf)), toEntityPlayer(player));
    }
