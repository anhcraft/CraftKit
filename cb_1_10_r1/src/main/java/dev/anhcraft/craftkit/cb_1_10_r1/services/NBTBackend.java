...
public class NBTBackend extends CBModule implements CBNBTBackend {
    ...
    private static NBTTag fromNMS(NBTBase tag){
        switch (tag.getTypeId()) {
            ...
            case TagType.DOUBLE_TAG: return new DoubleTag(((NBTTagDouble) tag).h());
    ...
    public void save(Block block, boolean replace) {
        CraftWorld craftWorld = (CraftWorld) block.getWorld();
        if(replace) {
            craftWorld.getHandle().setTileEntity(new BlockPosition(block.getX(), block.getY(), block.getZ()), TileEntity.a(craftWorld.getHandle(), root));
        } else {
