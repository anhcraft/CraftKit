...
public class NBTBackend extends CBModule implements CBNBTBackend {
    ...
    private static NBTTag fromNMS(NBTBase tag){
        switch (tag.getTypeId()) {
            ...
            case TagType.STRING_TAG: return new StringTag(tag.b_());
            ...
            case TagType.LONG_ARRAY_TAG: return new LongArrayTag(((NBTTagLongArray) tag).d());
        }
    ...
    public ItemStack save(ItemStack item) {
        ...
        ReflectionUtil.invokeDeclaredMethod(net.minecraft.server.v1_13_R1.ItemStack.class, i, "load",
    ...
    public void save(Block block, boolean replace) {
        CraftWorld craftWorld = (CraftWorld) block.getWorld();
        if(replace) {
            craftWorld.getHandle().setTileEntity(new BlockPosition(block.getX(), block.getY(), block.getZ()), TileEntity.create(root, craftWorld.getHandle()));
        } else {
