...
public class NBTBackend extends CBModule implements CBNBTBackend {
    ...
    public ItemStack save(ItemStack item) {
        ...
        ReflectionUtil.invokeDeclaredMethod(net.minecraft.server.v1_16_R3.ItemStack.class, i, "load",
    ...
    public void save(Block block, boolean replace) {
        ...
        CraftWorld craftWorld = craftBlock.getCraftWorld();
        if(replace) {
            craftWorld.getHandle().setTileEntity(new BlockPosition(block.getX(), block.getY(), block.getZ()), TileEntity.create(craftBlock.getNMS(), root));
        } else {
            TileEntity tileEntity = craftWorld.getHandle().getTileEntity(new BlockPosition(block.getX(), block.getY(), block.getZ()));
            if (tileEntity != null) {
                tileEntity.load(craftBlock.getNMS(), root);
            }
        }
    }
